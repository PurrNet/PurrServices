...
namespace PurrNet.Services
{
    public class AuthService
    {
        ...
        /// <summary>
        /// Signs in with a Steam Web API ticket (hex encoded) for <see cref="SteamTicketIdentity"/>.
        /// The server asks Steam who the ticket belongs to, so the player id is
        /// <c>steam:&lt;steamid64&gt;</c> and the name is their Steam persona name
        /// (<paramref name="displayName"/> is only a fallback). Requires the Steam provider to be
        /// enabled and configured (App ID + publisher Web API key) on the project's Auth page.
